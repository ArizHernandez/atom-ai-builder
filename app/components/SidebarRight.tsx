...
export default function SidebarRight() {
  ...
  return (
    <div className="relative flex z-20 shrink-0 h-full">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="absolute top-6 -left-3 z-50 flex items-center justify-center size-6 bg-white dark:bg-[#1b1e27] border border-slate-200 dark:border-[#282c39] rounded-full shadow-sm text-slate-500 hover:text-[#2559f4] transition-colors"
      >
      ...
      <aside
        className={`flex flex-col border-slate-200 dark:border-[#282c39] bg-white dark:bg-[#1b1e27] transition-all duration-300 ease-in-out overflow-hidden shadow-xl ${isExpanded ? 'w-[360px] border-l' : 'w-0 border-l-0 shadow-none'
          }`}
      >
            ...
            {messages.map((msg, index) => {
              ...
              return (
                <div key={index} className="flex gap-3">
                  <div
                    className={`size-8 rounded-full flex items-center justify-center shrink-0 shadow-lg ${isStreaming && isLastMessage
                  ...
                  <div className="flex flex-col gap-1 max-w-[85%]">
                    ...
                    <div
                      className={`p-3 rounded-2xl rounded-tl-none text-sm shadow-sm leading-relaxed whitespace-pre-wrap ${isStreaming && isLastMessage && isEmpty
