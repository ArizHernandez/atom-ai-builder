...
function RagToggle({
  ...
  return (
    ...
    >
      <div
        className={`mt-0.5 size-4 rounded flex items-center justify-center shrink-0 border transition-colors ${
          checked ? 'bg-[#2559f4] border-[#2559f4]' : 'border-slate-300 dark:border-slate-600'
        }`}
      >
        {checked && (
...
export default function PropertiesPanel() {
  ...
  const handleSave = () => {
    ...
    updateNodeConfig(node.id, {
      ...
      ...(hasRag         && { use_inventory: useInventory, use_faqs: useFaqs, use_agenda: useAgenda }),
    ...
    <div className="flex flex-col h-full">
      ...
      {!isReadOnly && (
        <div className="p-4 border-t border-slate-200 dark:border-[#282c39] shrink-0">
          <button
            onClick={handleSave}
            className={`w-full flex items-center justify-center gap-2 h-9 rounded-lg text-sm font-bold transition-all ${
              saved
